export default function animateHome() {
  const baseColorsHSV = [90, 195, 315, 50];
  ...
  let config = {
    SIM_RESOLUTION: 256,
    DYE_RESOLUTION: 256, //1440,
    CAPTURE_RESOLUTION: 256, // NOT USED.
    DENSITY_DISSIPATION: 2,
    VELOCITY_DISSIPATION: 0.5,
    PRESSURE: 0.1,
    PRESSURE_ITERATIONS: 20,
    CURL: 3, // Vorticity, 0-50, waves.
    SPLAT_RADIUS: 0.3, // Radius of splat.
    SPLAT_FORCE: 6000,
    SHADING: true,
    COLORFUL: true,
...
`
  ...
  let ditheringTexture = createTextureAsync('../public/LDR_LLL1_0.png');
  ...
  updateKeywords();
  initFramebuffers();
  // multipleSplats(parseInt(Math.random() * 20) + 5); // Initial explosion
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (e.code === 'KeyN') splatStack.push(parseInt(Math.random() * 30) + 5);
  });
  ...
  function generateColor() {
    // Select a random base color from the list
    const baseColor =
      baseColorsHSV[Math.floor(Math.random() * baseColorsHSV.length)];
    let c = HSVtoRGB(baseColor / 360, 0.5, 0.5); // Color / 360 bc "h" is in degrees, 0.5 is saturation and value/brightness
    c.r *= 0.15; // Multiply by 0.15 to make the color darker
    c.g *= 0.15;
    c.b *= 0.15;
    return c;
  }
